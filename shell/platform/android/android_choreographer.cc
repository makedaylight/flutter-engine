...
AndroidChoreographer::WhichChoreographer() {
  ...
  if (get_instance_fn && post_vsync_callback_fn && get_frame_time_nanos_fn &&
    ...
    FML_LOG(ERROR) << "Using NDK choreographer with frame timeline";
    ...
    FML_LOG(ERROR) << "Using NDK choreographer without frame timeline";
    AChoreographer_getInstance = get_instance_fn.value();
    ...
    FML_LOG(ERROR) << "Not using NDK choreographer";
