...
void VsyncWaiterAndroid::OnVsyncFromNDK(int64_t frame_nanos, void* data) {
  ...
  ConsumePendingCallback(weak_this, frame_time, target_time, 0);
}
...
void VsyncWaiterAndroid::OnVsyncFromNDK33(
  ...
  TRACE_EVENT3_INT("flutter", "PlatformVsync", "frame_start_time",
                   frame_time.ToEpochDelta().ToMicroseconds(),
                   "frame_target_time",
                   target_time.ToEpochDelta().ToMicroseconds(),
                   "frame_target_vsync_id", vsync_id);

  ConsumePendingCallback(weak_this, frame_time, target_time, vsync_id);
}
